...
export default function StudentLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 flex">
      ...
      <div
        className={`bg-white/95 backdrop-blur-xl w-80 shadow-2xl fixed lg:static h-full transform transition-all duration-500 z-50 border-r border-gray-200/60 flex flex-col ${isSidebarOpen
        ...
        <nav className="p-6 space-y-3 flex-1 overflow-y-auto">
          {menuItems.map((item) => (
            <Link
              ...
              className={`group flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 border border-transparent hover:border-blue-200/50 ${location.pathname === item.path
      ...
      <div className="flex-1 lg:ml-0 min-w-0 flex flex-col">
        {/* Enhanced Header */}
        <header
          className={`bg-white/80 backdrop-blur-xl border-b border-gray-200/60 lg:static sticky top-0 z-40 transition-all duration-300 ${scrolled ? "shadow-lg shadow-black/5" : "shadow-sm"
            }`}
        >
